# LocalCloud v0.39.3 — Release Notes & Quickstart
...
This is a patch release focused on **LocalCloud Manager (UI)** polish and reliability.

### LocalCloud Manager UI
- Improved log streaming for both **native** and **external** instances (cleaner log lines; WARN no longer misclassified as ERROR).
- Resources page: added service filtering, refresh, and smoother navigation (clickable rows + better scroll behavior).
- Demos: added search, improved output drawer layout, and reduced startup-race flakiness when loading demos.
- Terminal + code viewer: terminal now stays in the main layout (sidebar preserved) with theme selection; code viewer styling no longer shows faint horizontal lines / misaligned line numbers.
- Settings/About: clarified copy (“Everything is local”), improved naming (“LocalCloud Manager”), and more accurate version display.

### Core (Venus)
- Installer: disk checks no longer block installs when using `--skip-images`.
- Interception/networking: fixed container DNS binding using the correct port (avoids port `0`) and improved subnet validation/error behavior.
- RDS: subnet group responses now include `SubnetOutpost` for closer AWS parity.
...
### Install LocalCloud components for v0.39.3
...
```bash
localcloud install --version 0.39.3
```

**System-wide (recommended for shared servers):**
```bash
sudo localcloud install --system --version 0.39.3
```

**Headless servers (skip the UI):**
```bash
sudo localcloud install --system --no-ui --version 0.39.3
```

**Custom install prefix:**
```bash
localcloud install --prefix ~/localcloud --version 0.39.3
```
...
- `https://releases.localcloud.dev/0.39.3/manifest.json` (pinned release)
...
localcloud install ec2 -c ~/.localcloud/config/config.yml --release 0.39.3
```
